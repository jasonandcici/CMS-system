...
<?= $form->field($searchModel, 'nickname')->label('昵称') ?>
<?= $form->field($searchModel, 'is_enable')->dropDownList([1=>'启用',0=>'禁用'], ['prompt'=>'—不限—','class'=>'form-control'])->label('状态') ?>
<!-- 表单控件结束 -->
            ...
            <table class="table table-hover" id="list_data">
                ...
                <?php foreach($dataList = $dataProvider->models as $item){ ?>
                    <tr>
                        ...
                        <td><?=Html::encode($item->userProfile->nickname)?></td>
                        <td>
                            <?=$item->cellphone?$item->cellphone_code.' - '.Html::encode($item->cellphone):'——'?>
                        </td>
                        ...
                        <?php } if($userAccessButton['update'] || $userAccessButton['delete']){?>
                            <td class="opt" align="center">
                                <?php if($userAccessButton['view']){?>
                                    <?= Html::a(Yii::t('common','View'), ['view', 'id' => $item->id], ['class' => 'text-primary j_dialog_open','data-title'=>'用户“'.$item->username.'”详情','data-button'=>'false']) ?>
