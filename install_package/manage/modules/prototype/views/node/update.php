...
if($model->status === 2){
    $this->params['subTitle'] .='<span class="label label-primary" style="margin-left: 10px; font-size: inherit;">草稿</span>';
}elseif ($model->status === 4){
    $this->params['subTitle'] .='<span class="label label-warning" style="margin-left: 10px; font-size: inherit;">待审核</span>';
}elseif ($model->status === 3){
    $this->params['subTitle'] .='<span class="label label-info" style="margin-left: 10px; font-size: inherit;">已回收</span>';
}elseif ($model->status === 5){
$this->params['subTitle'] .='<span class="label label-danger" style="margin-left: 10px; font-size: inherit;">已驳回</span>';
}
...
<?= Html::a(Yii::t('common','Back List'),'javascript:history.go(-1);', ['class' => 'btn btn-default']) ?>
<?php $this->endBlock(); ?>
<!-- tab开始 -->
...
<div class="panel panel-default form-data <?=$model->status == 3?' form-recycle-bin':''?>">
    <div class="panel-body">
        ...
        <div class="tab-pane active" id="tab-content-base" role="tabpanel">
	        ...
	        <?php
	        if ( $model->status == 3 ) {
		        echo \common\helpers\HtmlHelper::activeHiddenInput( $model, 'status' );
	        } elseif ( $model->status == 2 ) {
		        echo $form->field( $model, 'status' )->radioList( [
			        1 => '发布',
			        2 => '草稿'
		        ], [ 'itemOptions' => [ 'labelOptions' => [ 'class' => 'radio-inline' ] ] ] )->label( '状态' );
	        } else {
		        echo $form->field( $model, 'status' )->radioList( [
			        1 => '启用',
			        0 => '禁用'
        ...
        <div class="tab-pane" id="tab-content-set" role="tabpanel">
            ...
            <?= $form->field($model, 'is_login')->radioList([1=>'是',0=>'否'],['itemOptions'=>['labelOptions'=>['class'=>'radio-inline']]]);?>
            ...
            <?=$form->field($model, 'layouts')->dropDownList(ArrayHelper::unifyKeyValue(ArrayHelper::getValue($templateList,'layouts',[])),['prompt'=>'禁用布局']);?>
